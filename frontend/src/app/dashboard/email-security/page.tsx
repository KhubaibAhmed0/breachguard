...
export default function EmailSecurityPage() {
  ...
  const getStatusBadge = (status?: string) => {
    switch (status?.toLowerCase()) {
      case 'pass':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold tracking-wider font-mono bg-emerald-500/10 text-emerald-300 border border-emerald-500/30 shadow-[0_0_12px_rgba(16,185,129,0.15)] backdrop-blur-sm">
      ...
      case 'warning':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold tracking-wider font-mono bg-amber-500/10 text-amber-300 border border-amber-500/40 shadow-[0_0_14px_rgba(245,158,11,0.2)] backdrop-blur-sm">
      ...
      case 'fail':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold tracking-wider font-mono bg-rose-500/10 text-rose-300 border border-rose-500/40 shadow-[0_0_14px_rgba(244,63,94,0.2)] backdrop-blur-sm">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium tracking-wider font-mono bg-zinc-900/90 text-zinc-400 border border-zinc-700/80">
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {loading ? (
          <div className="py-16 text-center text-zinc-500 text-xs font-roboto">Loading email security posture...</div>
        ) : !active ? (
          ...
          <>
                ...
                <div className="p-6 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm">
                  <div className="space-y-1.5">
                    ...
                    <h2 className="text-lg font-bold text-white tracking-tight flex items-center gap-2.5">
                      ...
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold tracking-wider font-mono border ${badge.classes}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className={`p-4 rounded-xl space-y-3 transition-all ${
                ...
                {active.spf?.record && (
                  <div className="p-2.5 bg-zinc-950 border border-zinc-800/90 rounded-lg text-[11px] font-mono text-zinc-300 break-all leading-relaxed">
                    {active.spf.record}
                  </div>
                )}
              </div>
              ...
              <div className="p-4 bg-zinc-900/40 border border-zinc-800/80 rounded-xl space-y-3">
                ...
                {active.mx?.records && active.mx.records.length > 0 && (
                  <div className="text-[11px] font-mono text-zinc-400 space-y-1 bg-zinc-950/60 p-2.5 rounded-lg border border-zinc-800/80">
              ...
              <div className="p-4 bg-zinc-900/40 border border-zinc-800/80 rounded-xl space-y-3">
                ...
                <div className="text-[11px] text-zinc-500 font-mono bg-zinc-950/60 p-2 rounded border border-zinc-800/60">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-semibold uppercase bg-amber-500/20 text-amber-300 border border-amber-500/40 shadow-[0_0_8px_rgba(245,158,11,0.2)]">
                          High Priority Action
                        </span>
                      </div>
                      <p className="text-xs text-zinc-300 mt-1 font-roboto leading-relaxed">
                        Your active policy is <code className="px-1.5 py-0.5 bg-zinc-900 border border-zinc-700/80 rounded text-amber-300 font-mono font-semibold">p={active.dmarc?.policy || 'none'}</code> (monitoring only). Inbound mail servers accept fraudulent spoofed messages claiming to originate from <span className="font-mono text-white">{active.domain}</span>.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-1 font-roboto">
                  <div className="p-3.5 bg-zinc-950/80 border border-zinc-800/90 rounded-xl space-y-2">
                    <div className="flex items-center gap-2 text-amber-300 text-xs font-semibold">
                      <span className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-[10px] font-mono text-amber-300 shrink-0">1</span>
                      <span>Audit Telemetry</span>
                    </div>
                    <p className="text-[11px] text-zinc-400 leading-snug">
                      Inspect inbound aggregate XML reports to catalog all authorized senders:
                    </p>
                    <code className="text-[10px] font-mono text-zinc-200 block bg-zinc-900 p-2 rounded-lg border border-zinc-800 break-all">
                  ...
                  <div className="p-3.5 bg-zinc-950/80 border border-zinc-800/90 rounded-xl space-y-2">
                    <div className="flex items-center gap-2 text-amber-300 text-xs font-semibold">
                      <span className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-[10px] font-mono text-amber-300 shrink-0">2</span>
                      <span>Sender Alignment</span>
                    </div>
                    <p className="text-[11px] text-zinc-400 leading-snug">
                      Verify SPF and DKIM alignment across Google Workspace, Office 365, and marketing relays.
                    </p>
                    <span className="inline-flex items-center gap-1.5 text-[10px] font-mono font-semibold text-emerald-300 bg-emerald-500/10 px-2.5 py-0.5 rounded-full border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.12)]">
                  ...
                  <div className="p-3.5 bg-zinc-950/80 border border-zinc-800/90 rounded-xl space-y-2">
                    <div className="flex items-center gap-2 text-amber-300 text-xs font-semibold">
                      <span className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-[10px] font-mono text-amber-300 shrink-0">3</span>
                      <span>Quarantine Policy</span>
                    </div>
                    <p className="text-[11px] text-zinc-400 leading-snug">
                      Route unauthenticated spoofed messages directly to recipient spam/junk folders:
                    </p>
                    <code className="text-[10px] font-mono text-amber-200 block bg-zinc-900 p-2 rounded-lg border border-amber-500/30 break-all">
                  ...
                  <div className="p-3.5 bg-zinc-950/80 border border-zinc-800/90 rounded-xl space-y-2">
                    <div className="flex items-center gap-2 text-emerald-400 text-xs font-semibold">
                      <span className="w-5 h-5 rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center text-[10px] font-mono text-emerald-300 shrink-0">4</span>
                      <span>Full Rejection (Zero Trust)</span>
                    </div>
                    <p className="text-[11px] text-zinc-400 leading-snug">
                      Instruct all global mail relays to immediately reject spoofed emails before delivery:
                    </p>
                    <code className="text-[10px] font-mono text-emerald-300 block bg-zinc-900 p-2 rounded-lg border border-emerald-500/30 break-all font-semibold">
