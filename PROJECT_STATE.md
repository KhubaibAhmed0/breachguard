...
11. **Founder Growth Hub (`/admin/growth`)**:
    ...
    * **Automated Social Buyer Intent Radar**: Continuously monitors Reddit (`r/sysadmin`, `r/msp`, `r/cybersecurity`) and Twitter/X for acute cybersecurity pain points (DMARC `p=none` spoofing, exposed administrative ports, infostealer credentials, compliance audits). Automatically scores buyer intent ($0-100\%$), extracts corporate domains and company names, crafts bespoke value-first reply scripts, and provides 1-click conversion to Outreach Leads with immediate perimeter scans.
    * **Bi-directional Google Sheet Automation**: 1-click real-time webhook push (`POST /push-sheet`) and Google Sheets-ready CSV export (`GET /radar/export`) matching intake columns (`Company Name`, `Domain`, `Contact Email`, `Contact Name`, `Intent Score`, `Pain Category`, `Source URL`) to automate buyer identification into spreadsheets without paid API subscriptions.
